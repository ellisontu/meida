...
@interface RecordVideoModel ()<AVCaptureFileOutputRecordingDelegate>
...
@property (nonatomic, assign) RecordFlashState flashState;

@end

@implementation RecordVideoModel
...
#pragma mark - setup
- (void)setUpWithType:(VideoViewScaleType )type
{
...
- (void)setUpPreviewLayerWithType:(VideoViewScaleType )type
{
    CGRect rect = CGRectZero;
    switch (type) {
        case Type1X1:
            rect = CGRectMake(0, 0, SCR_WIDTH, SCR_WIDTH);
            break;
        case Type4X3:
            rect = CGRectMake(0, 0, SCR_WIDTH, SCR_WIDTH*4/3);
            break;
        case TypeFullScreen:
...
- (NSString *)videoFolder
{
    ...
    if (![LocalFileManager isExistFileAtPath:direc]) {
        [LocalFileManager createCachePathWithFolderName:VIDEO_FOLDER];
    }
    return direc;
}
//清空文件夹
- (void)clearFile
{
    ...
    NSString *direc = [cacheDir stringByAppendingPathComponent:VIDEO_FOLDER];
    if ([LocalFileManager isExistFileAtPath:direc]) {
        [LocalFileManager deleteFileAtPath:direc];
    }
...
{
    if (error) {
        XLog(@"function === %s, error == %@",__func__, error);
    }
...
- (void)cutVideoWithFinished:(void (^)(void))finished
{
    ...
    [exporter exportAsynchronouslyWithCompletionHandler:^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            MDDeviceAuth photoAuthStatus = [Util checkPhotoAuthorityStatus];
            if (photoAuthStatus == DeviceAuthDenied)
            {
                [self checkDevicePhotoDenyStatus];
            }
            else if (photoAuthStatus == DeviceAuthNotDetermined){
                MDWeakPtr(weakPtr, self);
                [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
                    if (status == PHAuthorizationStatusAuthorized ){
                        [weakPtr saveImageToLocal];
                    }
                }];
            }
            else{
...
- (void)checkDevicePhotoDenyStatus
{
    UIAlertController *alertViewCtrl = [UIAlertController alertControllerWithTitle:@"开启相册权限" message:@"请先开启您手机的相册储存权限，才能保存图片至相册" preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        [Util goToAppSystemSettings];
    }];
    ...
    [alertViewCtrl addAction:confirmAction];
}

#pragma mark - 保存图片
- (void)saveImageToLocal
{
    UIImageWriteToSavedPhotosAlbum(IMAGE(@"camera_icon"), self, @selector(image:didFinishSavingWithError:contextInfo:), NULL);
}

- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo;
{
//    self.finishDownloadCount++;
//
//    if (self.finishDownloadCount < self.gallaryPics.count) {
//        [self dowloadImages];
//    }
//    if (self.finishDownloadCount == self.gallaryPics.count + 1) {
//        [_flowActivity stopAnimating];
//        //[Util showMessage:@"文案复制成功 海报已保存至相册" inView:XHCAPPDELEGATE.window];
//        [Util showMessage:@"文案复制成功 商品图片及海报已保存至相册" inView:XHCAPPDELEGATE.window];
//    }
}
