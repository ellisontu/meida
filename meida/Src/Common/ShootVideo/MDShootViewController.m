...
#import "MDAnimationRecordView.h"
#import "MDShootPlayerView.h"
#import "UIImage+Additions.h"

typedef NS_ENUM(NSInteger,ShootType) {
    ShootTypeImage,
    ShootTypeVideo,
};


@interface MDShootViewController ()<AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate,UIGestureRecognizerDelegate>
...
@property (nonatomic , strong) MDAnimationRecordView * recordView;
@property (nonatomic , strong) MDShootPlayerView *playerView;
@property (nonatomic , strong) UIImageView * shootImageView;
@property (nonatomic , strong) UIImage * shootImage;
@property (nonatomic , assign) ShootType shootType;
...
@property (nonatomic , strong) VideoModel * currentRecord;
...
@implementation MDShootViewController
...
#pragma mark --- init
- (void) setupViews {
    self.view.backgroundColor = COLOR_HEX_STR(@"0xffffff");
    ...
    self.recordView = [[MDAnimationRecordView alloc] init];
    ...
    [self.tipLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(self.recordView.mas_top).offset(0);
    }];
    ...
    [self.recordBackBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.center.equalTo(self.recordView);
    }];
...
- (void)setupVideo {
    ...
    if (![[MDShootManager shareManager] judgeIsHaveShootVideoAuthorityWithCallBackViewController:self]) {
    ...
    self.recodingQueue = dispatch_queue_create("com..queue", DISPATCH_QUEUE_SERIAL);
...
- (void)resetBtnLayout
{
    ...
    [self startRunning];
    [MDShootManager deleteVideo:self.currentRecord.videoAbsolutePath];
    ...
    [UIView animateWithDuration:0.25 animations:^{
        [self.view layoutIfNeeded];
        self.recordSureBtn.alpha = 0.;
        self.recordBackBtn.alpha = 0.;
...
- (void) recordSureAction {
    ...
    if (self.shootType == ShootTypeImage) {
        if ( [PHPhotoLibrary authorizationStatus] != PHAuthorizationStatusDenied) {
            UIImageWriteToSavedPhotosAlbum(self.shootImage, self, @selector(image:didFinisavingWithError:contextInfo:), NULL);
        }
...
// 保存图片的回调
- (void)image:(UIImage *)image didFinisavingWithError:(NSError *)error contextInfo:(void *)contextInfo{
}
...
- (void) startShootVideo {
    self.currentRecord = [MDShootManager createNewVideo];
...
- (void) endShootVideoWithTime: (CFTimeInterval) recordTime{
    ...
    if (recordTime < kPhotoTime) {
        self.shootType = ShootTypeImage;
        [self addImageView];
        
    }else{
        MDWeakPtr(weakPtr, self);
        [self saveVideo:^(NSURL *outFileURL) {
            [weakPtr addPlayerView];
            weakPtr.shootType = ShootTypeVideo;
        }];
...
- (void)addPlayerView {
    ...
    self.playerView = [[MDShootPlayerView alloc] initWithFrame:self.videoView.bounds videoUrl:videoURL];
...
- (void)saveVideo:(void(^)(NSURL *outFileURL))complier {
    ...
    dispatch_async(self.recodingQueue, ^{
        ...
        [MDShootManager saveThumImageWithVideoURL:outputFileURL second:0.1];
        
        if (weakPtr.assetWriter.status != AVAssetWriterStatusUnknown) {
            [weakPtr.assetWriter finishWritingWithCompletionHandler:^{
                
                if (weakPtr.currentRecordIsCancel) return ;
                
                [MDShootManager saveThumImageWithVideoURL:outputFileURL second:1];
...
- (UIButton *)cancelBtn {
    if (!_cancelBtn) {
        ...
        [_cancelBtn setTitleColor:COLOR_HEX_STR(@"0x7d8496") forState:UIControlStateNormal];
...
- (UIButton *)switchCameraBtn {
    if (!_switchCameraBtn) {
        ...
        [_switchCameraBtn setImage:ImageFromBundle(@"record_swichCamera") forState:UIControlStateNormal];
...
- (UIButton *)recordBackBtn
{
    if(!_recordBackBtn){
        ...
        [_recordBackBtn setImage:ImageFromBundle(@"record_shootBack") forState:UIControlStateNormal];
...
- (UIButton *)recordSureBtn
{
    if(!_recordSureBtn){
        ...
        [_recordSureBtn setImage:ImageFromBundle(@"record_shootSure") forState:UIControlStateNormal];
...
- (UILabel *)tipLabel {
    if (!_tipLabel) {
        ...
        _tipLabel.textColor = COLOR_HEX_STR(@"0x7d8496");
