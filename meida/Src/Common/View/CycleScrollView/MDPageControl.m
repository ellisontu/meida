...
- (void)initialization
{
    self.dotViewClass           = [MDAnimatedDotView class];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touch.view != self) {
        ...
        if ([self.delegate respondsToSelector:@selector(MDPageControl:didSelectPageAtIndex:)]) {
            [self.delegate MDPageControl:self didSelectPageAtIndex:index];
        }
    }
}
...
- (UIView *)generateDotView
{
    ...
    if (self.dotViewClass) {
        ...
        if ([dotView isKindOfClass:[MDAnimatedDotView class]] && self.dotColor) {
            ((MDAnimatedDotView *)dotView).dotColor = self.dotColor;
        }
    } else {
...
{
    if (self.dotViewClass) {
        MDAbstractDotView *abstractDotView = (MDAbstractDotView *)[self.dots objectAtIndex:index];
        if ([abstractDotView respondsToSelector:@selector(changeActivityState:)]) {
            [abstractDotView changeActivityState:active];
        } else {
            NSLog(@"Custom view : %@ must implement an 'changeActivityState' method or you can subclass %@ to help you.", self.dotViewClass, [MDAbstractDotView class]);
        }
